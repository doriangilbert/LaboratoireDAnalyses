...
public class Main extends Application {
	...
	public static void peuplementTables() {
		...
		DatabaseLink.getSession().saveOrUpdate(patient2);

		Analyse analyse1 = new Analyse("Hemogramme", 50);
		DatabaseLink.getSession().saveOrUpdate(analyse1);
		Analyse analyse2 = new Analyse("Groupe sanguin", 25);
		DatabaseLink.getSession().saveOrUpdate(analyse2);
		Analyse analyse3 = new Analyse("Vitesse de sédimentation", 75);
		DatabaseLink.getSession().saveOrUpdate(analyse3);

		Medecin medecin1 = new Medecin(147852369147852L, "Jean", "DUPONT", 3000);
		DatabaseLink.getSession().saveOrUpdate(medecin1);
		...
		DatabaseLink.getSession().saveOrUpdate(paiement1);
